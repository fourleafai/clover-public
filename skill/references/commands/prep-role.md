...
1. If the user gave a role + optional company, use it. If not, ask: "Which role, and at what company (if specific)?"
2. Call `get_role_intelligence({role})` for the pipeline + scoring dimensions.
3. Call `explain_interview_format({role, experienceLevel?, company?})` for the prescriptive walk-through.
4. Present in this order:
   - **Pipeline** — the rounds, what each tests
...
> "Six rounds at Stripe PM. Recruiter → hiring manager → product sense → analytical → strategy → behavioral. Three things to know:
>
> **What to expect:** Heavy on product sense and strategy rounds, light on technical. Stripe loves the "design a product for [niche audience]" prompt and wants you to push back on assumptions, not just answer.
>
> **How to win at senior:** Custom frameworks per case, not CIRCLES. Drive the case, don't follow. Bring your own metrics. Cite recent Stripe launches when they're relevant — shows you actually use the product.
>
...
- Don't read out the entire ROLE_INTELLIGENCE record. Synthesize.
- Don't recommend generic "do a mock interview" without offering the next concrete step.
