...
Clone the repo and build the per-tool bundles:
...
`npm run build` reads `SKILL.md` plus the `references/` tree and writes a `dist/<tool>/` directory for each supported tool, laid out exactly the way that tool expects. Then copy your tool's bundle into place:
...
Cursor requires the Nightly channel with **Settings → Rules → Agent Skills** enabled. The other tools work out of the box.
...
LICENSE                      MIT
package.json                 build script wiring
scripts/
...
- **`npx four-leaf-coach add`** npm CLI that auto-detects your tool and installs in one command, no clone or copy step. The `dist/` build pipeline is the foundation for it.
...
Done so far: per-tool `dist/` bundles generated from this source (`npm run build`), including a flattened single-file variant for GitHub Copilot.
